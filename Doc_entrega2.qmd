...
mpg %>% 
  ...
  ggplot() + geom_col(aes(x = class, y = cty_media, fill = class)) +
  labs(x = "Tipo de vehículo", y = "Rend. medio en ciudad (mpg) ") +
  theme_bw() + 
  guides(fill = guide_legend(title = "Vehículos")) +
...
mpg %>% 
  ...
  ggplot() + geom_col(aes(x = class_drv, y = cty_media, fill = class_drv), alpha = 0.8, 
                 position = "identity",
                 na.rm = TRUE) +
...
mpg %>% 
  ...
  ggplot() + geom_point(aes(x = displ, y = hwy, color = class), position = "identity", na.rm = TRUE) +
  facet_grid(.~class) +
  scale_fill_manual(values = c("green", "blue", "red", "darkorange","purple","cyan4", "grey")) +
  labs(x = "Tamaño del motor",
       y = "Rend. en carretera (en mpg)") +
...
mpg %>%
  ...
  ggplot() + geom_histogram(aes(x = cty, fill = drv),
                            alpha = 0.5, 
                            position = "identity",
                            na.rm = TRUE) +
  facet_grid(.~drv) +
