...
colores_degradados <- colorRampPalette(c("cyan3", "purple"))(length(unique(mpg2$manufacturer)))
...
colores_degradados2 <- colorRampPalette(c("cyan3", "purple", "red", "yellow")) (length(unique(mpg2$manufacturer)))
colores_degradados2
...
mpg2 %>%
  ...
  group_by(drv) %>% 
  ggplot() + 
  geom_histogram(aes(x = cty, fill = drv), alpha = 0.7, position = "identity",
                 na.rm = TRUE, bins=30) +
  facet_grid(.~drv) +
